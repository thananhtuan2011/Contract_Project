...
@Component({
  selector: 'app-load-thu',
  templateUrl: './load-thu.component.html',
  styleUrls: ['./load-thu.component.scss']
})
export class LoadThuComponent
  implements
  OnInit,
  OnDestroy,
  ICreateAction,
  ...
  searchtext: string;
  search_date: any = ''
  apithuchi = this.api + "/api/thuchi/getAllBill"
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.searchForm();
    const filter = {};
    this._Thu_chiService.patchStateBill(filter, this.apithuchi);
  ...
  sort(column: string) {
    ...
    this._Thu_chiService.patchStateBill({ sorting }, this.apithuchi);
  }

  // pagination
  paginate(paginator: PaginatorState) {
    this._Thu_chiService.patchStateBill({ paginator }, this.apithuchi);
  }
  ...
  editthu(id) {
    const modalRef = this.modalService.open(UpdateThuComponent, { size: 'xl' });
    modalRef.componentInstance.id = id;
    modalRef.result.then(() =>
      this._Thu_chiService.patchStateBill({}, this.apithuchi),
      () => { }
    );
    // 
  }
  ...
  edit(id: number) {
    const modalRef = this.modalService.open(CreatedThuComponent, { size: 'xl' });
    modalRef.componentInstance.id = id;
    modalRef.result.then(() =>
      this._Thu_chiService.patchStateBill({}, this.apithuchi),
      () => { }
  ...
  delete(id) {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (!res) {
        ...
        this._Thu_chiService.deleteThu(id).subscribe(res => {
          if (res) {
            this._Thu_chiService.patchStateBill({}, this.apithuchi);
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (!res) {
        ...
        this.grouping.getSelectedRows().forEach(element => {
          ...
          this._Thu_chiService.deleteThu(element).subscribe(res => {
            if (res) {
              this._Thu_chiService.patchStateBill({}, this.apithuchi);
            }
