...
@Component({
  selector: 'app-load-chi-plan',
  templateUrl: './load-chi-plan.component.html',
  styleUrls: ['./load-chi-plan.component.scss']
})

export class LoadChiPlanComponent
  implements
  OnInit,
  OnDestroy,
  ICreateAction,
  ...
  paginate_chi(paginator: PaginatorState) {
    this._Thu_chiService.patchStatechi({ paginator }, this.apichi);
  }

  create_planchi() {
    this.edit_chi(undefined);
  }
  edit_chi(id: number) {
    const modalRef = this.modalService.open(AddPlanChiComponent, { size: 'xl' });
    modalRef.componentInstance.id = id;
    modalRef.result.then(() =>
      this._Thu_chiService.patchStatechi({}, this.apichi),
      () => { }
    );
  }
  // form actions
  ...
  deleteSelected() {
    ...
    this.grouping.getSelectedRows().forEach(element => {
      this._Thu_chiService.deletePlanChi(element).subscribe(res => {
        if (res) {
          this._Thu_chiService.patchStatechi({}, this.apichi);
        }
    ...
    // modalRef.result.then(() => this._Thu_chiService.fetchchi(), () => { });
  }
