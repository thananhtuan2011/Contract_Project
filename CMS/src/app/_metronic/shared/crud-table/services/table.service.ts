...
export abstract class TableService<T> {
  ...
  public _itemsPartner$ = new BehaviorSubject<T[]>([]);
  ...
  public patchStateAllPartner(patch: Partial<ITableState>, apiRoute: string = '') {
    this.patchStateWithoutFetch(patch);
    this.fetch_AllPartner(apiRoute, "");
  }
  ...
  public fetch_AllPartner(apiRoute: string = '', nameKey: string = 'id') {
    var resItems: any = [];
    var resTotalRow: number = 0;
    this._isLoading$.next(true);
    this._errorMessage.next('');
    const request = this.find_AllPartner(this._tableState$.value, apiRoute)
      .pipe(
        tap((res: any) => {
          if (res) {
            resItems = res.data;
          ...
          this._itemsPartner$.next(resItems);
          this.__responseData$.next(res);
          this.patchStateWithoutFetch({
            paginator: this._tableState$.value.paginator.recalculatePaginator(
              resTotalRow
  ...
  find_AllPartner(tableState: ITableState, routeFind: string = '',): Observable<any> {
    const url = routeFind;
    const httpHeader = this.getHttpHeaders();
    this._errorMessage.next('');
    return this.http.post<any>(url, tableState, { headers: httpHeader }).pipe(
